...
export function SettingsApp() {
  ...
  const filteredLanguages = useMemo(() => languages.filter(([, name]) => name.toLowerCase().includes(languageSearch.toLowerCase())), [languageSearch]);
  ...
  return <main className="min-h-screen bg-[color:var(--cv-surface)] p-6 text-[color:var(--cv-text)]">
    ...
    <div className="mx-auto grid max-w-3xl gap-4">
      ...
      <section className="settings-card"><h2>Transcription language</h2>{settings.overrides.language && <p className="override">Environment override active: <code>CODEX_VOICE_LANG={settings.overrides.language}</code></p>}<label className="block text-sm">Search common languages<input className="field mt-1" value={languageSearch} onChange={event => setLanguageSearch(event.target.value)} placeholder="Search languages" /></label><select className="field mt-2" value={settings.language} onChange={event => void save("language", event.target.value)}>{filteredLanguages.map(([code, name]) => <option value={code} key={code}>{name}{code === "auto" ? " (default)" : ` (${code})`}</option>)}</select><label className="mt-3 block text-sm">Manual language code<input className="field mt-1" value={settings.language === "auto" || languages.some(([code]) => code === settings.language) ? "" : settings.language} placeholder="e.g. en-gb" onBlur={event => { if (event.target.value) void save("language", event.target.value); }} /></label><p className="muted">Automatic detection omits the language hint. Explicit codes are sent as hints. Detection uses an undocumented upstream endpoint, so accuracy and supported languages are not a stable API.</p></section>
    </div>
