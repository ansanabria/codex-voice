...
import { parseHistory, parseSettings, parseStatus } from "./contract.js";
...
describe("desktop protocol", () => {
  ...
  it("accepts valid transcript history", () => expect(parseHistory(JSON.stringify({ schemaVersion: 1, entries: [{ id: 1, createdAt: 123, text: "hello" }], hasMore: false })).entries[0]?.text).toBe("hello"));
  it("rejects malformed transcript history", () => expect(() => parseHistory(JSON.stringify({ schemaVersion: 1, entries: [{ id: "1", createdAt: 123, text: "hello" }], hasMore: false }))).toThrow());
